...
特点：
- 串行调用（用户选 A）
- 断点续跑：增量保存 JSON，崩溃后能接着跑
- 输出解析鲁棒：处理 'real'/'fake'/'真'/'假' 多种回复
- 重试机制：API 429/超时自动 retry 3 次
...
def call_deepseek(prompt: str, api_key: str, max_retries: int = 3) -> str:
    ...
    payload = {
        ...
        "temperature": 0.0,  # 零样本判别，确定性输出
        "max_tokens": 8,     # 只需要 real/fake 一个词
    }
