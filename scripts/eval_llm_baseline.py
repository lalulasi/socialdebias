...
def call_deepseek(prompt: str, api_key: str, model: str, max_retries: int = 3) -> str:
...
def main():
    ...
    if out_path.exists():
        with open(out_path, "r", encoding="utf-8") as f:
            cached = json.load(f)
        if (cached.get("pkl") == args.pkl
                and cached.get("sample") == args.sample
    ...
    for i in range(start_i, len(texts)):
        ...
        raw = call_deepseek(prompt, api_key, args.model)
