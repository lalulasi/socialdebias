...
class SurfaceFeatureExtractor:
    ...
    EMOTION_WORDS = {
        "anger": {
            ...
            "annoy", "annoyed", "annoying", "frustrated", "frustrating",
        },
        "joy": {
            ...
            "exciting", "excited", "wonderful", "fantastic", "great", "awesome",
        },
        "fear": {
        ...
        "disgust": {
            ...
            "dirty", "rotten", "yuck", "ugh", "outrageous",
        },
        "sadness": {
            ...
            "lonely", "loneliness", "regret", "regretful",
        },
        "surprise": {
            ...
            "remarkable", "extraordinary", "phenomenal", "spectacular",
        },
        "trust": {
            ...
            "proven", "established", "reputable", "legitimate", "transparent",
        },
        "anticipation": {
            ...
            "foresee", "foresaw", "preview", "ahead", "tomorrow", "outlook",
        },
    }
    ...
    def extract(self, text: str, max_chars: int = 10000) -> np.ndarray:
        ...
        if sents:
            ...
            features.append(np.mean(sent_lens))
            features.append(max(sent_lens))
        else:
            features.extend([0.0, 0.0])
        features.append(text.count("?") / n_chars * 1000)
        features.append(text.count("!") / n_chars * 1000)
        passive_count = sum(1 for t in doc if t.dep_ == "nsubjpass")
        features.append(passive_count / n_sent)

        # ===== 词汇 4 维 =====
        features.append(len(set(tokens)) / n_tok)  # TTR
        features.append(sum(1 for t in tokens if t in self.SUPERLATIVES) / n_tok)
        features.append(sum(1 for t in tokens if t in self.MODALS) / n_tok)

        # Flesch reading ease（归一化到 [0, 1]）
        try:
            import textstat
            flesch = textstat.flesch_reading_ease(text)
            features.append(max(0.0, min(1.0, (flesch + 100) / 200)))
        except Exception:
            features.append(0.5)
