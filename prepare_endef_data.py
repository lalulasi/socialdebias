...
ENDEF 每条样本需要:
  - content    : 新闻正文
  - label      : ENDEF_en 使用 0(real) / 1(fake)；ENDEF_ch 使用 "real" / "fake"
...
用法（在 socialdebias 项目根目录运行，需要 spaCy / jieba）:
  ...
  python prepare_endef_data.py --dataset weibo21 --repair_ch_labels_only
                                                    # 仅修复已生成的中文 JSON 标签
"""
import argparse
import json
...
# ============ 单 split 转换 ============
def encode_endef_label(label, lang):
    """Convert the repository's 0/1 convention to each ENDEF loader format."""
    if isinstance(label, str) and label.lower() in {"real", "fake"}:
        numeric_label = 0 if label.lower() == "real" else 1
    else:
        try:
            numeric_label = int(label)
        except (TypeError, ValueError) as exc:
            raise ValueError(f"无法识别的标签: {label!r}") from exc
    if numeric_label not in {0, 1}:
        raise ValueError(f"标签必须是 0/1 或 real/fake，实际为: {label!r}")
    return numeric_label if lang == "en" else ("fake" if numeric_label == 1 else "real")
...
def convert_split(texts, labels, lang, split_name):
    ...
    for i, (text, label) in enumerate(zip(texts, labels)):
        ...
        records.append({
            "content": str(text),
            "label": encode_endef_label(label, lang),
...
def repair_chinese_labels(out_dir):
    """Repair existing ENDEF_ch JSON labels without rerunning entity extraction."""
    out_dir = Path(out_dir)
    for split_name in ("train", "val", "test"):
        json_path = out_dir / f"{split_name}.json"
        if not json_path.is_file():
            raise FileNotFoundError(f"缺少待修复文件: {json_path}")
        with json_path.open("r", encoding="utf-8") as f:
            records = json.load(f)
        if not isinstance(records, list):
            raise ValueError(f"{json_path} 顶层必须是 JSON list")

        counts = {"real": 0, "fake": 0}
        for index, record in enumerate(records):
            if not isinstance(record, dict) or "label" not in record:
                raise ValueError(f"{json_path} 第 {index} 条缺少 label")
            label = encode_endef_label(record["label"], "zh")
            record["label"] = label
            counts[label] += 1
        ...
        print(
            f"  ✓ {split_name}: {len(records)} 条，"
            f"real={counts['real']} fake={counts['fake']} → {json_path}"
        )
    print("ENDEF_CH_LABELS_READY=True")
...
def process_dataset(name, cfg):
    ...
    for split_name, (texts, labels) in [
        ...
        n_fake = sum(1 for label in labels if encode_endef_label(label, "en") == 1)
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="仅把现有 data_weibo21 JSON 的 0/1 标签修复为 real/fake，不重新提取实体",
    )
    ...
    if args.repair_ch_labels_only:
        if args.dataset != "weibo21":
            parser.error("--repair_ch_labels_only 必须与 --dataset weibo21 一起使用")
        repair_chinese_labels(args.endef_ch_root / "data_weibo21")
        return
