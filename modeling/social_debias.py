...
class SocialDebiasModel(nn.Module):
    ...
    def __init__(
            ...
            use_comment_encoder: bool = False,  # 是否使用评论编码器（CHECKED）
            comment_model_name: str = "bert-base-chinese",  # 评论 BERT
        ...
        # ★ 门控融合：surface_feat → 投影 → 门控 → 768 维（保持 bias_projector 不变）
        # 评论编码器（可选，CHECKED 数据集用）
        ...
        if use_comment_encoder:
            ...
            # 评论融合门控：bert_hidden + comment_hidden → bert_hidden
        ...
        if surface_feat_dim > 0:
            ...
            self.surface_proj = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Dropout(dropout),
                nn.Linear(surface_hidden, bert_hidden),  # 投影到 768 维
            )
            self.fusion_gate = nn.Linear(bert_hidden * 2, 1)  # 门控权重
    ...
    def forward(self, input_ids, attention_mask, surface_feat=None, comment_input_ids=None, comment_attention_mask=None, comment_mask=None):
        ...
        # 事实分支（正常传播）
        # 评论融合（如启用）：把评论语义融入 shared 表示，再走 fact_branch
        if self.use_comment_encoder and comment_input_ids is not None:
            ...
            shared = gate * shared + (1 - gate) * comment_repr  # 融合后的 shared
        ...
        # ★ 如果有表层特征，拼接到偏置输入
        if self.surface_feat_dim > 0:
            if surface_feat is not None:
                surface_grl = surface_feat  # 表层特征不过 GRL
            else:
                # 训练对比学习 sub-epoch 时不带 surface_feat，用零向量补齐
                # （归一化后特征均值即 0，零向量代表"中性"特征）
                surface_grl = torch.zeros(
                    shared.size(0), self.surface_feat_dim,
                    device=shared.device, dtype=shared.dtype,
                )
            bias_input = torch.cat([bias_input, surface_grl], dim=-1)
        '''
        bias_input = self.grl(shared)  # [B, bert_hidden]

        # ★ 门控融合：surface_feat → 768 维 → 与 bert 加权融合
        if self.surface_feat_dim > 0 and surface_feat is not None:
            surface_proj = self.surface_proj(surface_feat)  # [B, 768]
            # 门控权重：[B, 1]
            gate = torch.sigmoid(
        ...
        # 如果 surface_feat is None（对比学习 sub-epoch），bias_input 保持 [B, 768] 不变
